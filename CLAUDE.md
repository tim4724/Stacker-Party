...
- Room state has ONE implementation: `server/RoomCore.js` (roster, auto-naming, colour slots, host election, pause/mute/results), loaded out of the same bundle by web, tvOS and Android TV. Don't re-implement any of it per platform — that drift is what the module exists to stop. It composes `partyplug/RoomFlow.js`, which stays game-agnostic. Mutators return a `publish` hint (`'now' | 'soon' | 'none'`); only the throttle timer is per-shell. Cross-platform gate: `tests/fixtures/room-core-golden.json`, regenerated with `npm run build:golden`

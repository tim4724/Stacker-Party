...
function renderLoop(timestamp) {
  ...
  if (displayGame && roomState === ROOM_STATE.PLAYING && !paused) {
    ...
    try {
      if (deltaMs > 0) {
        // One frame is one change. A tick that KOs three players at once (a
        // garbage cascade, a simultaneous top-out) would otherwise publish the
        // whole room three times before anyone sees the first. No floor: a frame
        // that moved nothing publishes nothing, which is what keeps this free at
        // 60 Hz. Mirrors the natives batching dispatchCommands().
        publishBatch(function () { stepEngine(deltaMs); });
      }

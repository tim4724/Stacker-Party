...
/// One roster row, decoded straight from the room brain's record
/// (`RoomBrain.list()` / `roomCall("list")`).
///
/// It used to be a mutable class the display wrote through; the brain owns every
...
public struct PlayerRecord: Decodable, Equatable {
    ...
    public let colorSlot: Int          // dense 0..MAX_PLAYERS-1 (the brain's `playerIndex`)
    public let startLevel: Int         // 1...15
    /// False between the relay's `peer_joined` and this player's own HELLO: the
    /// name and colour are our guesses until then. See RoomBrain.peerJoined.
    ...
    public init(from decoder: Decoder) throws {
        ...
        // The fixture path (RoomBrain.addPlayer) writes neither field; the brain's
